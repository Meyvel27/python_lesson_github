...
goal = 10       #стільки кораблів треба знищити
lost = 0        #рахунок пропущених кораблів
max_lost = 3    #програли, якщо пропустили 3х ворогів
life = 3
...
for i in range(1, 3):
    asteroid = Enemy(img_ast, randint(
        30, win_width - 30), -40, 80, 50,
        randint(1, 7))
...
while game:
    ...
    if not finish:
        ...
        if rel_time:     # if rel_time is True
            now_time = timer()
        ...
        for c in collides:
            score += 1
            monster = Enemy(img_enemy, randint(80, win_width - 80), -40,
                            80, 50, randint(1, 5))
            monsters.add(monster)
